...
async def _scrape_posts(username: str, apify_token: str, limit: int = 50) -> list[dict]:
...
def _preprocess_posts(posts: list[dict]) -> dict:
    captions = [
        {
...
def _generate_tov(username: str, posts: list[dict], anthropic_api_key: str) -> dict:
    data = _preprocess_posts(posts)
...
class InstagramTovService:
    ...
    async def analyze(self, username: str) -> dict:
        clean = _extract_username(username)
        if not clean:
            raise PrivateProfileError("Invalid username or URL")

        try:
            posts = await _scrape_posts(clean, self._apify_token)
        except Exception as e:
            raise ServiceError(str(e)) from e

        if not posts:
            raise NoPostsError("No posts found — profile may be private or have no captions")

        try:
            return _generate_tov(clean, posts, self._anthropic_api_key)
        except Exception as e:
            raise ServiceError(str(e)) from e
